%{
	...
	int bool_id_flag = 0;
	int in_func_flag = 0;
	struct enter_test* func_jump_stack;
	int func_jump_stack_count = 0;
	...
	void PopFuncJump ();
	void PushFuncJump (unsigned label, unsigned count);
	void ReplaceQuads (int a, int b);
	void Backpatch_Func_Jump (unsigned M);
...
expr:	assginexpr				{printf("%d: expr-> assginexpr\n", yylineno);
		...
		|expr DIV expr			{printf("%d: op-> DIV\n", yylineno);
									if($1->type == constnum_e && $3->type == constnum_e){
										...
										// x/0
										if($3->numConst == 0 && $3->intConst == 0) assert(0 && "divide with 0.\n");

										// 0/x
										else if ($1->numConst == 0 && $1->intConst == 0){
											$$->numConst = 0;
										}

										//int/x
										else if($1->numConst == 0){
											if ($3->numConst != 0){
												$$->numConst = $1->intConst / $3->numConst;
											}
											else{
												$$->numConst = $1->intConst / $3->intConst;
											}
										}
										//num/x
										else if($1->numConst != 0){
		...
		|expr GREATER expr			{printf("%d: op-> GREATER\n", yylineno);
										...
										quads[currQuad-1].taddress = currQuad+1;
		...
		|expr EQUAL expr			{printf("%d: op-> EQUAL\n", yylineno);
				...
				if(memFlag == 1){
					...
					printf("Error: can't assign in a function...\n");
								...
								if($lvalue->type == tableitem_e){
									...
									$assginexpr->sym->space = currscopespace();
									$assginexpr->sym->offset = currscopeoffset();
									incurrscopeoffset();
									printf("In assign:  space = %d, offset = %d\n", $assginexpr->sym->space, $assginexpr->sym->offset);
									emit(assign, $lvalue, NULL, $assginexpr, currQuad, yylineno);
								}
							}
			;

term:	MINUS expr  		{printf("%d: term-> UMINUS  expr\n", yylineno);
											check_arith($expr, yylval.strVal);
											$term = newexpr(arithexpr_e);
											struct expr* num = newexpr_constnum(-1);
											$term->sym = newtemp();
											$term->sym->space = currscopespace();
											$term->sym->offset = currscopeoffset();
											incurrscopeoffset();
											emit(mul, $expr, num, $term, currQuad, yylineno);
										}
...
lvalue:	 member 						{printf("%d: lvalue-> member\n", yylineno);
		...
		|ID 							{printf("%d: lvalue-> id\n", yylineno);
											...
											if(scope == 0){
												// Insert
												if(LookupInScopeVar(scope, yylval.strVal, "var") == -1){
											...
											// Any other scope.
												...
												if(in_func_block_flag > 0){
													if(LookupInScopeVar(0, yylval.strVal, "var") == -1){
														//Error - 
														...
														// Insert
														else if(offset = LookupInScopeVar(scope, yylval.strVal, "var") == -1){
																$lvalue = newexpr(var_e);
																sym = InsertVar(hashTable, ++key, yylval.strVal, scope, yylineno, 1);
																flagError = 2;
															}
															// Formal
															else if(LookUpinScopeFormal(scope,  yylval.strVal) != 0){
																	flagError = 3;
																	
															}else{
																flagError = 4;
																offset = LookupInScopeVar(scope, yylval.strVal, "var");
															}
														
													}else{
																
																if( LookupInScopeVar(0, yylval.strVal, "var") != -1){
																	offset = LookupInScopeVar(0, yylval.strVal, "var");
																	flagError = 5;
																}else{
																	offset = LookupInScopeVar(scope, yylval.strVal, "var");
																	flagError = 4;
												...
												// Insert
													...
													if(Lookup("var", yylval.strVal, scope) == -1){
														sym = InsertVar(hashTable, ++key, yylval.strVal, scope, yylineno, 1);
														flagError = 2;
													}
												}
											...
											if(flagError != 1){
												...
												if(flagError == 4){	
													$lvalue->sym->offset = offset;
													$lvalue->sym->space = functionlocal;
													$lvalue->sym->type = LOCAL;
													$lvalue->type = var_e;
												}
												if(flagError == 5){	
													$lvalue->sym->offset = offset;
													$lvalue->sym->space = programvar;
													$lvalue->sym->type = LOCAL;
													$lvalue->type = var_e;
												}
												if (flagError == 0){ 
...
call:	call L_PAREN	{
							paramCount = 0;
																				...
																				$$ = make_call($1, elists, currQuad, yylineno);
...
block:	 L_BRACKET {scope++; NewScope(scope); } stmts {} R_BRACKET	{Hide (scope);  scope--; printf("%d: block-> { stmts } scope = %d\n", yylineno, scope); $block = $stmts;}
...
funcprefix: FUNCTION funcname { printf("%d: funcprefix, $funcname = %s\n", yylineno, $funcname);
								if(LookUpinScopeFunc(scope, $funcname, "func") == 0){
									$funcprefix = newexpr(programfunc_e);
									...
									func_jump_stack_count = 0;
									AddFuncname ($funcname, scope);

									PushFuncJump(currQuad, func_jump_stack_count);
									emit(jump, NULL, NULL, NULL, currQuad, yylineno);

									emit(funcstart, $funcprefix, NULL, NULL, currQuad, yylineno);
									func_jump_stack_count++;
								}
								else{
...
ifstmt: ifprefix stmt {printf("%d: ifstmt-> ifprefix stmt\n", yylineno);
						...
						quads[$ifprefix].taddress = currQuad;
						$ifstmt = $stmt;
						bool_flag = 0;
						bool_op_flag = 0;
						}
		...
		|ifprefix stmt elseprefix stmt{printf("%d: ifstmt-> ifprefix stmt elseprefix stmt\n", yylineno);
										...
										quads[$ifprefix].taddress = $elseprefix+1;
										bool_flag = 0;
										bool_op_flag = 0;
										
										patchlabel($elseprefix, currQuad);
										quads[$elseprefix].taddress = currQuad;
...
whilestmt: whilestart whilecond loopstmt {printf("%d: whilestmt-> while ( expr ) stmt\n", yylineno);
										...
										emit(jump, NULL, NULL, jump_int, currQuad, yylineno);
										quads[currQuad-1].taddress = $whilestart;
										
										patchlabel($whilecond, currQuad);
										quads[$whilecond].taddress = currQuad;
										if($loopstmt){
...
forstmt: forprefix N elistlist R_PARENR N loopstmt N {printf("%d: whilestmt-> forstmt-> for ( N elist; expr; elist ) N stmt N\n", yylineno);
													...
													quads[$1->enter].taddress = $5+1;

													patchlabel($2, currQuad);
													quads[$2].taddress = currQuad;

													patchlabel($5, $1->test);
													quads[$5].taddress = $1->test;

													patchlabel($7, $2+1);
													quads[$7].taddress = $2+1;
...
void Backpatch_Func_Jump (unsigned M){
	int count = func_jump_stack->test;
	int i = 0;

	for (i; i<count; i++){
		quads[func_jump_stack->enter].result = (struct expr*) malloc(sizeof(struct expr));
		quads[func_jump_stack->enter].result->intConst = M;
		quads[func_jump_stack->enter].taddress = M;

		PopFuncJump();
	}
		quads[func_jump_stack->enter].result = (struct expr*) malloc(sizeof(struct expr));
		quads[func_jump_stack->enter].result->intConst = M+1;
		quads[func_jump_stack->enter].taddress = M+1;
		PopFuncJump();
}
...
void PushFuncJump (unsigned label, unsigned count){
	struct enter_test* tmp = (struct enter_test*)malloc(sizeof(struct enter_test));

	tmp->enter = label;
	tmp->test = count;

	if(func_jump_stack == NULL){
		func_jump_stack = tmp;
	}
	else{
		tmp->next = func_jump_stack;
		func_jump_stack = tmp;
	}
}
...
void PopFuncJump (){
	struct enter_test* tmp = func_jump_stack;
	func_jump_stack = func_jump_stack->next;
}
...
void Init(){
	...
	func_jump_stack =  (struct enter_test*)malloc(sizeof(struct enter_test));
